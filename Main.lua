...
-- [[ COMBINED VERSION - ALL MODULES INTEGRATED ]]
...
local State = {
    ...
    SelectedResource = {}, 
    IsFarmingMob = false,
    SelectedMob = {}, -- [UBAH] Jadi Table kosong biar support multi-select
    CurrentTarget = nil,
    LastFlyHeight = nil
}
    ...
    pcall(function()
        for _, Obj in pairs(workspace:GetDescendants()) do
            ...
            if State.IsMining then
                -- Logic Ore (Sudah Multi)
                if table.find(State.SelectedResource, Obj.Name) then
                    local HP = Obj:GetAttribute("Health") or (Obj:FindFirstChild("HP") and Obj.HP.Value) or 0
                    if HP > 0 then 
                        Valid = true 
                        Pos = Obj:IsA("Model") and Obj:GetPivot().Position or Obj.Position
                    end
                end
                
            elseif State.IsFarmingMob then
                -- [FIX MULTI MOB]
                -- Kita cek satu-satu nama mob yang dipilih
                if Obj:IsA("Model") then
                    for _, TargetName in ipairs(State.SelectedMob) do
                        if Obj.Name:find(TargetName) then
                            ...
                            if Hum and Hum.Health > 0 and HRP then 
                                Valid = true
                                Pos = HRP.Position
                                break -- Kalau udah ketemu yg cocok, stop looping nama
                            end
                        end
                    end
                end
            ...
            if Valid and Pos then
                ...
                if Dist < State.Max_Distance then
...
local function StartMovementEngine()
    ...
    State.LastTargetY = nil
    LastScanTime = 0
    CurrentScanCooldown = 0.5
    ...
    EngineConnection = RunService.RenderStepped:Connect(function(dt)
        ...
        if State.CurrentTarget then
            local Dead = false
            if State.IsMining then
                local HP = State.CurrentTarget:GetAttribute("Health")
                    or (State.CurrentTarget:FindFirstChild("HP") and State.CurrentTarget.HP.Value)
                    or 0
            ...
            if Dead then State.CurrentTarget = nil end
        end

        -- SCAN TARGET LOGIC (Updated for Multi-Select Check)
        if not State.CurrentTarget or not State.CurrentTarget.Parent then
            -- Cek apakah Table Ore atau Table Mob ada isinya (Length > 0)
            local HasSelection = (State.IsMining and #State.SelectedResource > 0)
                or (State.IsFarmingMob and #State.SelectedMob > 0)

            if HasSelection and (os.clock() - LastScanTime) >= CurrentScanCooldown then
                local Found = GetNearest()
                State.CurrentTarget = Found
                CurrentScanCooldown = Found and 0.5 or 2.0
        ...
        if not State.CurrentTarget then
            if not State.LastTargetY then
                ...
                if result then
                    State.LastTargetY = result.Position.Y
                else
                    State.LastTargetY = Root.Position.Y - State.Height_Offset
                end
            end

            local IdleY = State.LastTargetY + State.Height_Offset
            local HoverPos = Vector3.new(Root.Position.X, IdleY, Root.Position.Z)

            local ImaginaryTarget = HoverPos - Vector3.new(0, State.Height_Offset, 0)
            if math.abs(State.Height_Offset) < 0.1 then
                ImaginaryTarget = HoverPos - Vector3.new(0, 5, 0)
            end

            Root.CFrame = CFrame.lookAt(HoverPos, ImaginaryTarget)
            return
        end

        -- [[ TARGET FOUND LOGIC ]]
        local TargetCenter
        if State.IsFarmingMob and State.CurrentTarget:FindFirstChild("HumanoidRootPart") then
            TargetCenter = State.CurrentTarget.HumanoidRootPart.Position
        else
            TargetCenter = State.CurrentTarget:IsA("Model")
                and State.CurrentTarget:GetPivot().Position
                or State.CurrentTarget.Position
        end
        
        State.LastTargetY = TargetCenter.Y 

        local Offset = State.Height_Offset
        if math.abs(Offset) < 0.05 then Offset = 0.05 end

        local GoalPos = TargetCenter + Vector3.new(0, Offset, 0)

        local LookDir = TargetCenter - GoalPos
        if LookDir.Magnitude < 0.01 then LookDir = Root.CFrame.LookVector end
        local LockedRotation = CFrame.lookAt(Vector3.zero, LookDir).Rotation

        local CurrentPos = Root.Position
        local Dist = (GoalPos - CurrentPos).Magnitude

        if Dist > 1 then
            local Dir = (GoalPos - CurrentPos).Unit
            local Step = Dir * (State.Travel_Speed * dt)
            local NewPos = (Step.Magnitude >= Dist) and GoalPos or (CurrentPos + Step)
            Root.CFrame = CFrame.new(NewPos) * LockedRotation
        else
            Root.CFrame = CFrame.new(GoalPos) * LockedRotation
        end
    end)
end
...
local function StartAttackEngine()
    ...
    AttackLoop = task.spawn(function()
        local ToolService = ReplicatedStorage:WaitForChild("Shared", 10):WaitForChild("Packages"):WaitForChild("Knit"):WaitForChild("Services"):WaitForChild("ToolService")
        local RF = ToolService and ToolService:WaitForChild("RF"):WaitForChild("ToolActivated")
        while (State.IsMining or State.IsFarmingMob) do
            if State.CurrentTarget and RF then
                pcall(function() RF:InvokeServer(State.IsMining and "Pickaxe" or "Weapon") end)
            end
            task.wait(0.1)
        end
    end)
...
function AutoFarm:Load(Parent, Palette, Engine) 
    Engine:CreateSection(Parent, "Farm Settings", 1)
    
    Engine:CreateSlider(Parent, "Height Offset", -10, 10, State.Height_Offset, 2, function(val) 
        State.Height_Offset = val 
    end)
    
    Engine:CreateSlider(Parent, "Tween Speed", 0, 100, State.Travel_Speed, 3, function(val) 
        State.Travel_Speed = val 
    end)

    -- ORES
    Engine:CreateSection(Parent, "Farm Ores", 5)
    local OreData = {
    ...
    Engine:CreateDropdown(Parent, "Select Island (Ore)", {"Stonewake's Cross", "Forgotten Kingdom", "Frostspire Expanse"}, 6, function(val)
        if CurrentResDropdown then pcall(function() CurrentResDropdown.Frame:Destroy() end) end
        CurrentResDropdown = Engine:CreateDropdown(Parent, "Select Rocks", OreData[val] or {}, 7, true, function(res) 
            State.SelectedResource = res 
        end)
    end)
    Engine:CreateToggle(Parent, "Start Farm Ore", 8, function(state)
        if state then 
            StopFarm()
            State.IsMining = true
            StartMovementEngine() 
    ...
    -- MONSTERS
    Engine:CreateSection(Parent, "Farm Monsters", 9)
    local MobData = {
        ["Stonewake's Cross"] = {"Zombie", "Elite Zombie", "Delver Zombie", "Brute Zombie"},
    ...
    Engine:CreateDropdown(Parent, "Select Island (Mob)", {"Stonewake's Cross", "Forgotten Kingdom", "Frostspire Expanse"}, 10, function(val)
        if CurrentMobDropdown then pcall(function() CurrentMobDropdown.Frame:Destroy() end) end
        
        -- [UBAH] Multiselect jadi true
        CurrentMobDropdown = Engine:CreateDropdown(Parent, "Select Monster", MobData[val] or {}, 11, true, function(mob) 
            State.SelectedMob = mob 
        end)
    end)
    Engine:CreateToggle(Parent, "Start Farm Mobs", 12, function(state)
        if state then 
            StopFarm() 
...
function Esp:Load(Parent, Palette, Engine) 
    ...
    Engine:CreateSlider(Parent, "Render Dist", 500, 1000, 700, 100, function(Val)
        RENDER_DISTANCE = Val
    end)
end
...
task.spawn(function()
    ...
    Dashboard:Load(DashPage, Engine.Palette, Engine)
